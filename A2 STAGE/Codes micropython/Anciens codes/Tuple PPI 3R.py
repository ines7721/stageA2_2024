...
def setup(resolution_ADC, resolution_timer, prescaler_binaire, comparaison_compteur) :
    ...
    mem32[BASE_ADDR_ADC + 0x518 + (0 * 0x10)] = 0x0000000 #burst mode off pour AIN0
    ...
    mem32[BASE_ADDR_ADC + 0x518 + (7 * 0x10)] = 0x0000000
...
def calcul_resistance(ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U, total_sample, R2) :
    for j in range(0, nb_resistances) :
        for i in range (0, nb_sur_echantillons) : 
            tension_U2_analog.append(ADC(Pin(PIN_U2[j], Pin.IN)))
            TST.value(1)
            tensionU2 += (tension_U2_analog[i].read_u16(),)
            TST.value(0)
            floatref = float(ref[j-1])
            U2 += (tensionU2[i] * (floatref / 65535),)
            tension_U_analog.append((ADC(Pin(PIN_U[j], Pin.IN))))
            TST.value(1)
            tensionU += (tension_U_analog[i].read_u16(),)
            TST.value(0)
            U += (tensionU[i] * (floatref / 65535),)
...
if frequence_echantillonnage in range (8000, 200001) : 
    ...
    #Nombre de tics d'horloge à laisser passer avant que le compteur ne déclenche un nouvel échantillon. Permet de baisser la fréquence d'échantillonnage (A VERIFIER)
...
while i < 20000 :
    ref = tension_ref(PIN_U, ref)
    R2, total_sample = calcul_resistance(ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U, total_sample, R2)
    #print("Résistances inconnues des circuits 1, 2, 3 :", R2)
    historique_resistances += (R2,)
    R2 = ()
    ref = ()
