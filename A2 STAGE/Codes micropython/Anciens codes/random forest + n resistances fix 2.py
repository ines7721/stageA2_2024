...
def calcul_parametres (R2, alerte, nb_resistances) :
    for k in range (0, nb_donnees_pour_classif) :
        R_slice = slice(R2, nb_resistances)
        integrale.append(calcul_integrale(nb_resistances, debut, R2, k))
        ...
        for i in range (0, nb_resistances) :
            print("integrale[i][k] :", integrale[i][k], "type integrale :", type(integrale), "type integrale[i][k] :", type(integrale[i][k]))
            print("type delta :", type(delta))
            print("delta :", delta)
            print("type delta[i][k] :", type(delta[i][k]))
            print("abs(delta[i][k]) :", abs(delta[i][k]), "type delta :", type(delta), "type abs(delta[i][k]) :", type(abs(delta[i][k])))
            print("abs(derivee_max[i][k]) :", abs(derivee_max[i][k]), "type derivee_max :", type(derivee_max), "type abs(derivee_max[i][k]) :", abs(derivee_max[i][k]))
            somme[i][k] = integrale[i][k] + abs(delta[i][k]) + abs(derivee_max[i][k])
