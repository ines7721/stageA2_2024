...
def setup(resolution_ADC, resolution_timer, prescaler_binaire, comparaison_compteur) :
    ...
    mem32[BASE_ADDR_TIMER + 0x000] = 0 #mise en mode Timer
    mem32[BASE_ADDR_TIMER + 0x508] = prescaler_binaire #définition de la fréquence d'échantillonnage
    mem32[BASE_ADDR_TIMER + 0x540 + (0 * 0x4)] = 01  #CC[0], comparaison déclenchée pour la valeur de compteur comparaison_compteur 
    #2) CONFIG CANAL PPI / EEP & TEP
    mem32[BASE_ADDR_PPI + 0x500] = 1 #activation du canal 0 de périphérique PPI
    mem32[BASE_ADDR_PPI + 0x510 + (0 * 0x8)] = BASE_ADDR_TIMER + 0x140 + (0 * 0x4) #initialisation de l'élément déclencheur du PPI : channel 0 EEP -> EVENTS_COMPARE[0] du registre timer
    mem32[BASE_ADDR_ADC + 0x5F8] = 0000000000000000 #sample rate contrôlé par tâche SAMPLE uniquement
    mem32[BASE_ADDR_PPI + 0x514 + (0 * 0x8)] = BASE_ADDR_ADC + 0x004 #initialisation de l'élément déclenché du PPI : channel 0 TEP -> TASKS_SAMPLE du registre SAADC
    mem32[BASE_ADDR_PPI + 0x910 + (0 * 0x4)] = BASE_ADDR_TIMER + 0x008 #initialisation du second élément déclenché du channel PPI 0 : timer réinitialisé
...
if frequence_echantillonnage in range (8000, 200001) : 
    prescaler = round(32768 / frequence_echantillonnage) - 1
...
derivee_max_r_test = ()
integrale_r_test = ()
#Arrêt du compteur RTC sinon les changement de fréquence ne seront pas pris en compte :
