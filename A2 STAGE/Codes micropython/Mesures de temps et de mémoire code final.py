...
def print_memory_time(function_name, start_time, start_mem, end_time, end_mem):
    elapsed_time = (end_time - start_time) * 1e-6  # Convert microseconds to seconds
    memory_used = start_mem - end_mem
    print(f"{function_name} - Temps: {elapsed_time:.6f} s")
    print(f"{function_name} - Mémoire libre avant: {start_mem} octets")
    print(f"{function_name} - Mémoire libre après: {end_mem} octets")
    print(f"{function_name} - Mémoire utilisée: {memory_used} octets")
...
def setup(resolution_ADC, nb_etalonnages, PIN_alim, nb_echantillons_par_montee, duree_montee_s) :
    ...
    end_mem = gc.mem_free()
    print_memory_time('set up', start_time, start_mem, end_time, end_mem)
...
def etalonnage(nb_etalonnages) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('etalonnage', start_time, start_mem, end_time, end_mem)
...
def tension_ref(PIN_U) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('tension ref', start_time, start_mem, end_time, end_mem)
    return ref
...
def calcul_resistance(ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U, alerte, attente_us, nb_resistances) :
    ...
    for z in range(nb_resistances, nb_resistances+10) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    #print_memory_time('calcul_resistance', start_time, start_mem, end_time, end_mem)
    return R2, alerte
...
def clear_calcul_resistances(tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U):
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    #print_memory_time('clear_calcul_resistance', start_time, start_mem, end_time, end_mem)
...
def slice(R2, nb_resistances):
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('slice', start_time, start_mem, end_time, end_mem)
    return R_slice
...
def calcul_integrale (nb_resistances, debut, R2, k) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('calcul_integrale', start_time, start_mem, end_time, end_mem)
    return integrale
...
def calcul_delta (R_slice, nb_resistances) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('calcul_delta', start_time, start_mem, end_time, end_mem)
    return delta
...
def calcul_derivee_max (R_slice, debut, nb_resistances) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('calcul_derivee_max', start_time, start_mem, end_time, end_mem)
    return derivee_max
...
def calcul_parametres(R2, alerte, nb_resistances, ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U, attente_us, total_sample, nb_donnees_pour_classif, debut):
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('calcul_parametres', start_time, start_mem, end_time, end_mem)
...
def classification(R2, alerte, nb_resistances, ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U, attente_us, total_sample, nb_donnees_pour_classif, debut) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('classification', start_time, start_mem, end_time, end_mem)
    return DMMP
...
def main(ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U, alerte, attente_us, nb_resistances, total_sample) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('main', start_time, start_mem, end_time, end_mem)
...
def type_aleatoire() :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('type_aleatoire', start_time, start_mem, end_time, end_mem)
...
def type_aleatoire_pondere() :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('type_aleatoire_pondere', start_time, start_mem, end_time, end_mem)
    return random.choice(parametres)
...
def donnees_aleatoires(type_noeud, n, integralef, derivee_maxf, deltaf, sommef) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('donnees_aleatoires', start_time, start_mem, end_time, end_mem)
    return donnee
...
def verif_noeud(type_noeud, donnee) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('verif_noeud', start_time, start_mem, end_time, end_mem)
    return condition
...
def arbre_decisionnel(n, integralef, derivee_maxf, deltaf, sommef) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('arbre_decisionnel', start_time, start_mem, end_time, end_mem)
    return vote_arbre
...
def foret(integralef, derivee_maxf, deltaf, sommef) :
    ...
    end_time = utime.ticks_us()
    end_mem = gc.mem_free()
    print_memory_time('foret', start_time, start_mem, end_time, end_mem)
    return DMMP
...
while True :
    ...
    R2, alerte, total_sample = main(ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, U2, tension_U_analog, tensionU, U, alerte, attente_us, nb_resistances, total_sample)
    if alerte == True :
        print("Alerte == True dans le main")
