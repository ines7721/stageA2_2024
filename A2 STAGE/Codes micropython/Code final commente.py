...
parametre_etalonnage = 1000000000000 #nombre d'échantillons à effectuer avant d'étalonnger à nouveau
...
seuil_delta = [0.5, 0.7] #intervalle dans lequel une donnée de delta doit être comprise pour indiquer qu'il n'y a pas de danger
seuil_derivee_max = [0.5, 0.7] #intervalle dans lequel une donnée de dérivée doit être comprise pour indiquer qu'il n'y a pas de danger
seuil_integrale = [0.5, 0.7] #intervalle dans lequel une donnée d'intégrale doit être comprise pour indiquer qu'il n'y a pas de danger
seuil_somme = [1.5, 2.1] #intervalle dans lequel une donnée de somme des paramètres (integrale, derivée, delta) doit être comprise pour indiquer qu'il n'y a pas de danger
nb_arbres = 10
...
def calcul_parametres(R2, alerte, nb_resistances, ref, R1, PIN_U, PIN_U2, tension_U2_analog, tensionU2, tension_U_analog, tensionU, attente_us, total_sample, nb_donnees_pour_classif, debut):
    ...
    for k in range(nb_donnees_pour_classif):
        ...
        for i in range(nb_resistances):
            integralef[i].append(integrale[i][0]) #on ajoute la valeur d'intégrale calculée pour chaque résistance à la liste qui stockera toutes les données de paramètres
...
def type_aleatoire() :
    #On choisit le paramètre que le noeud va évaluer avec parametres = ("delta", "integrale", "derivee_max", "somme")
    return random.choice(parametres)


def type_aleatoire_pondere() :
    #le choix du paramètre que le noeud va évaluer dépend des valeurs de précision obtenue en fonction des paramètres utilisés avec l'algorithme random forest
...
def verif_noeud(type_noeud, donnee) :
    ...
    if type_noeud == 'delta' :
        condition = donnee > random.choice(seuil_delta) or donnee < random.choice(seuil_delta)
    if type_noeud == 'derivee_max' :
        condition = donnee > random.choice(seuil_derivee_max) or donnee < random.choice(seuil_delta)
    if type_noeud == 'integrale' :
        condition = donnee > random.choice(seuil_integrale) or donnee < random.choice(seuil_delta)
    if type_noeud == 'somme' :
        condition = donnee > random.choice(seuil_somme) or donnee < random.choice(seuil_delta)
    return condition
